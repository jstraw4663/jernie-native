...
| Timeline reorder | `react-native-reanimated-dnd` v2 | Version-compatible, but its `Sortable` owns a scroll container and its own list state, while this timeline already owns one animated scroll surface with nested day/band structure and live external updates. Built on the Gesture Handler + Reanimated already here: `Gesture.Pan().activateAfterLongPress(500)`, with pre-activation movement beyond Gesture Handler's 10-point allowance cancelling the pending drag so scrolling wins. Drops remain constrained to the same persisted `stopId + dayId`. The measured destination also drives a labeled insertion boundary, active-band tint, and the lifted row's live time-column preview. |
