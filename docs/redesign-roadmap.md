...
| **Itinerary timeline** (inserted) | **Tasks 1–6.6 done and device-confirmed; broader Task 7 device review remains.** A sprint between Sessions 6 and 7 with its own plan and spec — continuous timeline, date rail, maps chooser, swipe actions with delayed-commit Undo, and cross-day long-press reorder. Same- and cross-day drag, the smooth post-drop geometry close, and reduced-bounce tuning are device-confirmed. Tasks 6.2–6.6 keep layout, screen state, nested handlers, native stacking, and the lifted visual stable while Pan crosses sibling days or edge autoscrolls, then settles post-drop geometry without moving the viewport. See `docs/superpowers/plans/2026-08-24-itinerary-timeline.md`. |
| Session 7 — Explore | **Code complete, automated gate green** (`dc6302c` on `feat/session-7-explore`); device pass pending. Filter state lifted into `ExploreFilterContext` above `<Tabs>`; stop defaults current → next → last, never "all" with a trip. One filter bar, one carousel, a two-column grid. New `ChipDropdown` primitive. See `docs/superpowers/plans/2026-08-26-explore.md`. |
| Sessions 8–12 | Not started. **Next: Session 8 — Map**, a new fifth route that consumes Session 7's store. |
